...
const Dashboard: React.FC = () => {
  ...
  const getColorByRange = (score: number | null): string => {
    if (score === null) return "lightGray"; // Did not participate
    if (score <= 2) return "red"; // 1-2
    if (score <= 5) return "yellow"; // 3-5
    if (score <= 8) return theme.colors.lightGreen; // 6-8
    return theme.colors.seduloGreen; // 9-10
  };
          ...
          bar: {
            series: {
              ...
              itemStyler: (params) => {
                ...
                const fillColor = getColorByRange(score); // Determine color based on range
