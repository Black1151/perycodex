...
export default async function PerygonMain() {
    ...
    if (authToken) {
        try {
            const [fetchLoggedInUserData, fetchCarouselItems, fetchProfileStatus] = await Promise.all([
                apiClient(`/getView?view=vwLoggedInUserIdentity&selectColumns=role&userUniqueId=${uniqueId}`, {
                    method: "GET",
                    cache: "no-store",
                }),
