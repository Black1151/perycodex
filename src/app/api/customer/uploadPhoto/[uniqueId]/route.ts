...
export async function POST(req: NextRequest, { params }: { params: { uniqueId: string } }) {
    ...
    try {
        ...
        const laravelResponse = await fetch(`${process.env.BE_URL}/customer/${uniqueId}`, {
            method: 'POST', // HAS TO BE POST
            body: laravelFormData,
            headers: {
                'Authorization': `Bearer ${authToken}`, // Include your Laravel API token if needed
            },
        });
        ...
        if (laravelResponse.ok) {
            ...
            return NextResponse.json({ success: "File uploaded successfully.", data: laravelResponseBody });
        } else {
            const errorBody = await laravelResponse.text(); // Get error message from Laravel backend
            return NextResponse.json({ error: "Failed to upload file to Laravel backend.", details: errorBody }, { status: laravelResponse.status });
        }
    } catch (error) {
