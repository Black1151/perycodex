...
const NavigationSidebar: React.FC<NavigationSidebarProps> = ({menuItems, ...sidebarProps}) => {
    ...
    const groupedItems = internalItems.reduce((acc, item) => {
            ...
            {Object.entries(groupedItems).map(([category, items], catIdx) => (
                <React.Fragment key={category}>
                    ...
                    {catIdx < Object.keys(groupedItems).length - 1 && (
                        <Divider borderColor={theme.colors.perygonPink} opacity={0.8} my={6}/>
                    )}
                </React.Fragment>
            ))}
        </VStack>
