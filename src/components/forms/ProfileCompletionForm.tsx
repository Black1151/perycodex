...
// Define the expected shape of the response data from fetchClient
...
export function ProfileCompletionForm({
    ...
    const handleFormSubmit: SubmitHandler<ProfileCompletionFormInputs> = async (
        ...
        try {
            const res: UpdateUserResponse | null = await fetchClient("/api/user/updateUserDetails", {
                method: "PUT",
                body: updatedData,
            ...
            // Check if `isProfileRegistered` is true before redirecting
            if (res?.userData?.resource?.isProfileRegistered) {
                router.push("/"); // Redirect to the main page or home
            } else {
                console.warn("Profile is not yet fully registered.");
                // Optionally handle the case where profile setup is incomplete
            }
