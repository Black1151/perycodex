...
const SurveyTestComponent: React.FC<SurveyTestComponentProps> = ({cssFiles, sjsFiles, jsFiles}) => {
    const [isBuildingScreen, setIsBuildingScreen] = useState(true);
    const [surveyJson, setSurveyJson] = useState('');
    const [datasetJson, setDatasetJson] = useState('{}');
    const [layout, setLayout] = useState('default');
    const [endpoint, setEndpoint] = useState('');
    const [cssPath, setCssPath] = useState(cssFiles.includes('admin') ? 'admin' : '');
    const [sjsPath, setSjsPath] = useState(sjsFiles.includes('admin') ? 'admin' : '');
            ...
            parsedSurveyJson.pages?.forEach((page: any) => {
                page.elements?.forEach((element: any) => {
                    if (element.choicesByUrl?.url) {
                        element.choicesByUrl.url = element.choicesByUrl.url.replace("NEXT_PUBLIC_BASE_URL", baseUrl);
                    }
                });
            });
    ...
    const generateDatasetTemplate = () => {
        try {
            ...
            const datasetTemplate = parsedSurveyJson.pages?.reduce((acc: any, page: any) => {
                page.elements?.forEach((element: any) => {
                    ...
                    <VStack spacing={6} w="100%">
                        <FormControl isInvalid={jsonError}>
                            ...
                            <Textarea
                                ...
                                resize="vertical"
                                variant="filled"
                                _focus={{borderColor: theme.colors.perygonPink}}
                            />
                            {jsonError && (
                        ...
                        <FormControl isInvalid={datasetError}>
                            <FormLabel color="gray.600">Dataset JSON (Prefill Data)</FormLabel>
                            <Textarea
                        ...
                        <Button variant="primary" onClick={generateDatasetTemplate}
                        ...
                        >
                            {jsonError || datasetError ? "Correct JSON to Continue" : "Build Survey"}
                        </Button>
                    </VStack>
                ) : (
                    <Box w="100%">
