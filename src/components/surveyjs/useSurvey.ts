      ...
      if (jsPath) {
        try {
          ...
          const jsModule = await import(
            `@/components/surveyjs/jsPath/${jsPath}`
          );
          ...
          if (jsModule?.applyJsWithoutSurvey) {
            ...
            console.error(
              `applyJsWithoutSurvey function not found in module at path: ${jsPath}`,
            );
          }
          ...
          if (jsModule?.applyJsWithSurvey) {
            ...
            console.error(
              `applyJsWithSurvey function not found in module at path: ${jsPath}`,
            );
          }
        } catch (error) {
          console.error(
            `Error loading JS functions from path: ${jsPath}`,
            error,
          );
        }
      } else {
    ...
    const applyTheme = async (surveyModel: Model): Promise<void> => {
      if (sjsPath) {
        try {
          ...
          console.error(
            `Error loading SurveyJS theme from path: ${sjsPath}`,
            error,
          );
        }
      }
